# netcat [nc] 命令对 Linux 和 Unix 进行端口扫描
> 参考文章：
> 1. https://linux.cn/article-6733-1.html
...
## 第一部分 使用 netcat [nc] 命令对 Linux 和 Unix 进行端口扫描
...
```
### 1.2  `nc/netcat` 命令
`-z` 参数用来告诉 `nc` 报告开放的端口，而不是启动连接。在 `nc` 命令中使用 `-z` 参数时，你需要在`主机名/ip` 后面限定端口的范围和加速其运行：
```
### 语法 ###
...
```
### 2.21 使用 TCP ACK (PA) 和 TCP Syn (PS) 扫描远程主机
