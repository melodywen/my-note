...
| 层级 | 原则 | 调用频率 | Agent / Category | 模型 | 倍率 |
|------|------|----------|------------------|------|------|
| 🔴 关键 | 不省钱 | 极高 | sisyphus（总指挥） | claude-sonnet-4.6 | 2.00x |
| 🔴 关键 | 不省钱 | 低 | prometheus（规划） | claude-sonnet-4.6 | 2.00x |
| 🔴 关键 | 不省钱 | 低 | ultrabrain（深度思考） | claude-sonnet-4.6 | 2.00x |
| 🔴 关键 | 不省钱 | 低 | deep（深度研究） | claude-sonnet-4.6 | 2.00x |
| 🔴 关键 | 质量成本平衡 | 高 | sisyphus-junior（主力执行） | kimi-k2.6 | 0.50x |
| 🟡 中等 | 适度投入 | 低 | hephaestus（工匠） | gpt-5.4 | 1.65x |
| 🟡 中等 | 适度投入 | 极低 | oracle（顾问） | gpt-5.4 | 1.65x |
| 🟢 次要 | 尽量省钱 | 极低 | atlas（执行指挥） | deepseek-v4-pro | 0.13x |
| 🟢 次要 | 尽量省钱 | 极低 | metis（漏洞分析） | deepseek-v4-pro | 0.13x |
| 🟢 次要 | 尽量省钱 | 极低 | momus（严格审查） | deepseek-v4-pro | 0.13x |
| 🟢 次要 | 尽量省钱 | 中 | visual-engineering（前端） | deepseek-v4-pro | 0.13x |
| 🟢 次要 | 尽量省钱 | 低 | artistry（设计） | deepseek-v4-pro | 0.13x |
| 🟢 次要 | 尽量省钱 | 低 | writing（写作） | kimi-k2.6 | 0.50x |
| 🟢 次要 | 尽量省钱 | 高 | explore（代码搜索） | deepseek-v4-flash | 0.05x |
| 🟢 次要 | 尽量省钱 | 中 | librarian（文档搜索） | deepseek-v4-flash | 0.05x |
| 🟢 次要 | 尽量省钱 | 高 | quick（快速小活） | deepseek-v4-flash | 0.05x |
| 🟢 次要 | 尽量省钱 | 中 | unspecified-low（杂活） | deepseek-v4-flash | 0.05x |
| 🟢 次要 | 尽量省钱 | 低 | unspecified-high（重要未分类） | deepseek-v4-pro | 0.13x |
| 👁️ 特殊 | 看图唯一选择 | 极低 | multimodal-looker（看图） | gemini-3.1-pro | 1.32x |
