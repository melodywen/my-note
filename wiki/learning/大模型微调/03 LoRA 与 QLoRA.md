...
> [!key-insight] 一句话串起来
> 优势①是它的"立身之本"(省显存),②③④是顺带白拿的好处。其中④说明了**多业务部署**的杀手锏:一个量化底模 + N 个几十 MB 的小适配器,就能服务 N 个任务(呼应 [[02 全量微调与高效微调|全量微调与高效微调]] 里的多任务部署成本对比)。
...
> [!key-insight] 要不要动 Bias/LayerNorm?哪些参数才会把模型搞崩
> **结论:这两个绝大多数情况保持默认(都冻结)就行,是"可选锦上添花",不是必须。** PEFT 默认 `bias="none"` + LayerNorm 冻结已覆盖 90% 场景。新手先用默认全冻结跑通,效果不够再松绑。
> - **放开 Bias/LayerNorm 几乎不会搞崩**:它们参数极少(几万 vs 上千万),改动幅度小,LayerNorm 本身还是"稳定器",放开通常让训练更稳。属于**低风险旋钮**,顶多"没帮助",很难"搞崩"。
> - **真正容易搞崩的是这些"力度"类参数**:
>
> | 参数 | 调错的后果 | 安全做法 |
> |---|---|---|
> | **学习率 lr** | **最危险**:太大→loss 飞(NaN)、模型胡言乱语 | LoRA 常用 `1e-4 ~ 2e-4`,别乱加 |
> | **α/r 比值** | 太大→等效学习率过高,训练动荡 | `α=r` 或 `α=2r` |
> | **r(秩)** | 太大→过拟合+显存爆;太小→学不动 | 从 16 起步 |
> | **epoch 轮数** | 太多→过拟合,丢失通用能力 | 1~3 轮,看验证集 |
>
> 一句话:**想稳先动学习率,别先去折腾 Bias/LayerNorm。**
...
- 上一篇:[[02 全量微调与高效微调|全量微调与高效微调]](LoRA 属于 PEFT 的"重参数化"类)
- [[01 大模型微调概述|大模型微调概述]]
- [[05 灾难性遗忘|灾难性遗忘]](LoRA 冻结主体权重,天然缓解遗忘)
